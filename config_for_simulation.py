...
def _get_option_contract_index(opt_folder):
    ...
    with _OPTION_CONTRACT_INDEX_CACHE_LOCK:
        ...
        for file_path in file_paths:
            ...
            if not filename.lower().endswith((".parquet", ".csv")):
                continue
...
def _read_market_file(path, columns=None):
    """Read a local or Azure Blob CSV/Parquet file."""
    path = str(path)
    suffix = Path(path.split("?", 1)[0]).suffix.lower()

    if suffix == ".csv":
        if STORAGE_MODE == "blob":
            payload = download_blob_bytes(path)
            return pd.read_csv(
                io.BytesIO(payload),
                usecols=columns,
                low_memory=False,
...
def _find_market_file(folder, filenames):
    """Find a CSV/Parquet file under a local folder or Blob prefix."""
...
def load_index_data_by_symbol(
    ...
    """
    Load index data from week 166 and older layouts.

    Supported examples:
        <week>/IDX_TICK/NIFTY.parquet
        <week>/IDX_TICK/NIFTY.csv
        <week>/IDX_TICK/20260717/NIFTY.csv
        <week>/20260717/IDX_TICK/NIFTY.csv
        <week>/NSE_IDX_TICK_20260717/NIFTY.csv
    """
    ...
    if date_value:
        filenames.extend([
            f"{symbol_name}{date_value}.parquet",
            f"{symbol_name}{date_value}.csv",
            f"{symbol_name}_{date_value}.parquet",
            f"{symbol_name}_{date_value}.csv",
            f"{date_value}.parquet",
            f"{date_value}.csv",
        ])

    candidate_folders = _candidate_segment_folders(
        folder,
        IDX_SEGMENT_NAME,
        date_value,
    )
    if date_value:
        if STORAGE_MODE == "blob":
            candidate_folders.extend([
                _join_storage_path(folder, f"NSE_IDX_TICK_{date_value}"),
                _join_storage_path(folder, date_value),
            ])
        else:
            candidate_folders.extend([
                os.path.join(folder, f"NSE_IDX_TICK_{date_value}"),
                os.path.join(folder, date_value),
            ])

    candidate_folders = list(dict.fromkeys(str(x) for x in candidate_folders))

    for candidate_folder in candidate_folders:
        path = _find_market_file(candidate_folder, filenames)
        if not path:
            continue
        ...
        try:
            ...
            logger.warning(
                "Unable to read index file %s: %s",
                path,
                exc,
            )
            continue

        if date_value and frame is not None and not frame.empty:
            frame = frame[
                frame["datetime"].dt.strftime("%Y%m%d") == date_value
            ].reset_index(drop=True)

        if frame is not None and not frame.empty:
            logger.info(
                "Loaded %s index data for %s from %s (%d rows)",
                symbol_name,
                date_value,
                path,
                len(frame),
    ...
    logger.warning(
        "No index file found: symbol=%s date=%s week=%s candidates=%s",
        symbol_name,
        date_value,
        folder,
        candidate_folders,
...
def get_dates_for_week_folder(
    ...
    if STORAGE_MODE == "blob":
        ...
        # Week 166 may store one folder per date and CSV files. Read valid
        # YYYYMMDD tokens from folder/blob paths before opening any data file.
        try:
            ...
            logger.warning(
                "Unable to discover dates from blob paths under %s: %s",
                prefix,
                exc,
            )

        # -----------------------------------------------------
        # 1. Read trading dates from index CSV/Parquet files.
        ...
        index_candidates = []
        for candidate_prefix in _candidate_segment_folders(
            prefix,
            IDX_SEGMENT_NAME,
            None,
        ):
            for filename in (
                ...
                found = _find_market_file(candidate_prefix, filename)
                if found:
                    index_candidates.append(found)

        for idx_file in dict.fromkeys(index_candidates):
            try:
                d = _read_market_file(idx_file, columns=["date"])
                dates.update(_dates_from_frame(d))
            except Exception:
                try:
                    d = _read_market_file(idx_file)
                    dates.update(_dates_from_frame(d))
                except Exception as exc:
                    logger.warning(
                        "Unable to read index file %s for dates: %s",
                        idx_file,
                        exc,
                    )
